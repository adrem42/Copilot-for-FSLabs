/* MODULEUSER64.C	Internal User interface library for FSUIPC5
*******************************************************************************

Started:          11th July 2000, adapted for 64-bit 10th June 2017
...
{  // abort if already started
	...
	m_hWnd = FindWindowEx(NULL, NULL, TEXT("UIPCMAIN"), NULL);
	if (!m_hWnd)
	...
	m_pNext = m_pView + 8; // Allow space for code pointer
...
{	F64IPC_READSTATEDATA_HDR *pHdr = (F64IPC_READSTATEDATA_HDR *) m_pNext;
	...
	if (((m_pNext - m_pView) + 12 + (dwSize + sizeof(F64IPC_READSTATEDATA_HDR))) > m_ulMax)
