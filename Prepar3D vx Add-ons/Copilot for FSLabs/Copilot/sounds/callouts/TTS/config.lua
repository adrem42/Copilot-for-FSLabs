
return {

  isTTS = true,

  pressureZero = "pressure zero",
  thrustSet = "thrust set",
  oneHundred = "one hundered",
  V1 = "v one",
  rotate = "rotate",
  positiveClimb = "positive climb",
  spoilers = "spoilers",
  noSpoilers = "no spoilers",
  reverseGreen = "reverse green",
  noReverseLeft = "no reverse left",
  noReverseRight = "no reverse right",
  noReverse = "no reverse",
  decel = "desell",
  noDecel = "no desell",
  seventy = "seventy",
  fullLeft_1 = "full left",
  fullLeft_2 = "full left",
  fullRight_1 = "full right",
  fullRight_2 = "full right",
  fullUp = "full up",
  fullDown = "full down",
  neutral_1 = "neutral",
  neutral_2 = "neutral",
  neutral_3 = "neutral",
  flapsOne = "flaps one",
  flapsOne_speedChecked = "speed checked",
  flapsTwo = "flaps two",
  flapsTwo_speedChecked = "speed checked",
  flapsThree = "flaps three",
  flapsThree_speedChecked = "speed checked",
  flapsFull = "flaps full",
  flapsFull_speedChecked = "speed checked",
  flapsZero = "flaps zero",
  flapsZero_speedChecked = "speed checked",
  speedTooLow = "speed too low",
  speedTooHigh = "speed too high",

  checklists = {

    standingBy = "standing by",
    doubleCheck = "double check that please",
    
    beforeStart = {
      cockpitPrep = "cockpit prep",
      cockpitPrepCompleted = "completed",
      gearPinsAndCovers = "gear pins and covers",
      signs = "signs",
      adirs = "aydirs",
      fuelQuantity = "fuel quantity",
      toData = "takeoff data",
      baroRef = "baro ref",
      completed = "down to the line"
    },

    beforeStartBelow = {
      windowsDoors = "windows doors",
      closed = "closed",
      beacon = "beacon",
      thrustLevers = "thrustLevers",
      parkingBrake = "parkingBrake",
      completed = "before start checklist complete"
    },

    afterStart = {
      antiIce = "anti ice",
      ecamStatus = "ecam status",
      pitchTrim = "pitch trim",
      rudderTrim = "rudder trim",
      completed = "after start checklist complete"
    },

    beforeTakeoff = {
      flightControls = "flight controls",
      checked1 = "checked",
      checked2 = "checked",
      flightInstruments = "flight instruments",
      briefing = "briefing",
      flapSetting = "flap setting",
      conf1 = "config 1 plus f",
      conf2 = "config 2",
      conf3 = "config 3",
      takeoffSpeedsFlexTemp = "takeoff speeds flex temp",
      ATC = "atc",
      ecamMemo = "ecam memo",
      completed = "down to the line"
    },

    beforeTakeoffBelow = {
      takeoffRwy = "takeoff runway",
      cabinCrew = "cabin crew",
      tcas = "tcas",
      engModeSelector = "engine mode selector",
      packs = "packs",
      completed = "before takeoff checklist complete"
    },

    landing = {
      cabinCrew = "cabin crew",
      autoThrust = "auto thrust",
      autoBrake = "auto brake",
      ecamMemo = "ecam memo",
      completed = "landing checklist complete"
    },

    parking = {
      apuBleed = "APU bleed",
      engines = "engines",
      seatBelts = "seat belts",
      externalLights = "external lights",
      fuelPumps = "fuel pumps",
      parkingBrakeAndChocks = "parking brake chocks",
      completed = "parking checklsit complete"
    },

    securingTheAircraft = {
      adirs = "aydirs",
      oxygen = "oxygen",
      apuBleed = "apu bleed",
      emerExitLights = "emergency exit lights",
      signs = "signs",
      apuAndBat = "apu and battery",
      completed = "securing the aircraft checklist complete"
    }
  }
}