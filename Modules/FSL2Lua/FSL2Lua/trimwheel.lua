...
function trimwheel:_set(CG, step)
  local CG_man
  if CG then CG_man = true else CG = atsuLog:getMACTOW() or ipc.readDBL(0x2EF8) * 100 end
  if not CG then return
  else CG = tonumber(CG) end
  if not step then
    if not CG_man and prob(0.1) then util.sleep(plusminus(10000,0.5)) end
  ...
  repeat
    ...
    if CG > CG_ind then
      if dist > 3.1 then self:_set(CG_ind + 3,1) util.sleep(plusminus(350,0.2)) end
